...
macOS 版本使用系统文档窗口管理 Markdown 文件：

- 通过 `文件 > 打开` 或工具栏打开按钮打开现有 `.md`、`.markdown` 和纯文本文件。
- 使用 `Command-S` 保存，或通过 `文件 > 另存为` 保存副本。
- 每个文档拥有独立窗口；关闭未保存文档时，由系统显示确认提示。
- iPhone 版本仍以 Markdown 渲染示例列表作为入口。

### 编辑体验

- 工具栏提供 Edit、Split、Read 三种模式，分别显示源码、左右双栏和阅读预览。切换模式保留源码编辑器的光标与撤销历史。
- `Command-F` 打开原生查找栏，`Command-Shift-H` 打开替换栏，也可使用工具栏的查找和替换按钮。阅读模式下查找会切回双栏，在源码中检索。
- 查找栏支持逐项定位、替换和全部替换；编辑及替换可使用 `Command-Z` 撤销。
- 底部统计全文源码的词数、字符数、行数。词数使用系统分词（中文按词组识别）；字符数包含空白、换行与 Markdown 标记，复合 Emoji 计为一个字符；空文档为零行，末尾换行计入新的空行。
